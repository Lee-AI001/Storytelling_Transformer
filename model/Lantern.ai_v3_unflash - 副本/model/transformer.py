...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout):
        ...
        # Cache rotary matrices for common sequence lengths
        self.rotary_cache = {}
        for seq_len in [128, 256, 512]:
            self.rotary_cache[seq_len] = get_rotary_matrix(seq_len, d_model, torch.device('cpu'), theta_base=10000.0)
    ...
    def forward(self, tgt, tgt_mask=None, tgt_key_padding_mask=None):
        ...
        # Get rotary matrix from cache or compute
        rotary_matrix = self.rotary_cache.get(seq_len)
        if rotary_matrix is None:
            rotary_matrix = get_rotary_matrix(seq_len, d_model, device, theta_base=10000.0)
        else:
        ...
        q_rotated = apply_rotary_embeddings(q, rotary_matrix)
        k_rotated = apply_rotary_embeddings(k, rotary_matrix)

        # Create boolean causal mask for self-attention
        if tgt_mask is None:
            tgt_mask = torch.triu(torch.ones(seq_len, seq_len, dtype=torch.bool, device=device), diagonal=1)
            # In nn.MultiheadAttention, True means mask (ignore), False means attend
            # torch.triu with diagonal=1 sets upper triangle (future tokens) to True
        ...
        try:
            ...
            logging.error(f"Self-attention failed: {e}")
            raise RuntimeError(f"Self-attention failed: {e}")
