...
class DynamicBatchScheduler:
    ...
    def estimate_batch_size(self, model, dataset, max_len, device):
        ...
        try:
            ...
            logging.info(f"Estimated batch size: {batch_size} (memory/sample: {memory_per_sample:.2f}GB, available VRAM: {available_vram:.2f}GB)")
            return batch_size
...
class SGDRScheduler:
    ...
    def step(self):
        ...
        if self.current_epoch < self.warmup_epochs:
            # Warmup: Linearly scale LR from 0 to max_lr
    ...
    def load_state_dict(self, state_dict):
        ...
        self.warmup_epochs = state_dict.get('warmup_epochs', 0)  # Backward compatibility
        self.current_epoch = state_dict['current_epoch']
        self.cycle_mult = state_dict['cycle_mult']
        self.current_cycle = state_dict['current_cycle']
        self.cycle_progress = state_dict['cycle_progress']  # Fixed: Correct key
...
def evaluate(model, data_loader, criterion, genre_criterion, device, vocab_size, tokenizer, pad_idx=0):
    ...
    # Collect a random subset of 10 samples for ROUGE evaluation
    ...
    # Generate hypotheses for selected samples
    with torch.no_grad():
        for sample in selected_samples:
            input_ids = sample['input_ids'].unsqueeze(0).to(device)  # Shape: [1, seq_len]
            # Filter special tokens for reference
            ...
            # Generate hypothesis using sampling
            ...
            for _ in range(config.MAX_GEN_LEN):
                ...
                if next_token.item() == config.EOS_IDX:
                    break
            # Filter special tokens for hypothesis
    ...
    # Compute loss and genre loss on full validation set
...
def save_checkpoint(model, optimizer, epoch, train_loss, val_loss, checkpoint_dir, scheduler=None, warmup_scheduler=None, scaler=None):
...
def get_context_window_params(epoch):
    ...
    return schedule[0][1], schedule[0][2], schedule[0][3]  # Default to first stage
...
def train():
    ...
    # Updated SGDR scheduler with warmup integration
    scheduler = SGDRScheduler(
        optimizer,
        min_lr=config.SGDR_MIN_LR,
        max_lr=config.SGDR_MAX_LR,
        cycle_length=5,  # Reduced for more cycles
        warmup_epochs=config.WARMUP_EPOCHS
    ) if config.USE_SCHEDULER and config.SCHEDULER_TYPE == "sgdr" else None
    warmup_scheduler = None  # Warmup now handled in SGDRScheduler
    if config.SCHEDULER_TYPE == "cosine":
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
    ...
    for epoch in range(start_epoch, config.EPOCHS):
        try:
            ...
            if (train_dataset is None or 
                ...
                batch_size = batch_scheduler.estimate_batch_size(model, train_dataset, max_len, config.DEVICE) if batch_scheduler else config.BATCH_SIZE
                logging.info(f"Epoch {epoch+1}: Using batch size {batch_size}")
            ...
            for batch in progress_bar:
                ...
                with torch.amp.autocast(device_type='cuda', enabled=config.DEVICE.type == "cuda"):
                    outputs = model(
                        input_ids,
                        tgt_mask=None,  # Removed attention_mask
                ...
                progress_bar.set_postfix(loss=f"{batch_loss:.4f}", perplexity=f"{batch_perplexity:.2f}")
            ...
            logging.info(f"  Val Perplexity: {val_perplexity:.2f}, ROUGE-L: {val_rouge:.4f}")
            logging.info(f"  Genre Loss: {val_genre_loss:.4f}, LR: {current_lr:.6f}")
            print(f"\nEpoch {epoch+1}/{config.EPOCHS}")
            print(f"  Train Loss: {avg_train_loss:.4f}, Val Loss: {val_loss:.4f}")
            print(f"  Val Perplexity: {val_perplexity:.2f}, ROUGE-L: {val_rouge:.4f}")
            print(f"  Genre Loss: {val_genre_loss:.4f}, LR: {current_lr:.6f}")
